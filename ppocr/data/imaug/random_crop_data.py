...
class EastRandomCropData(object):
    ...
    def __call__(self, data):
        ...
        for attempt in range(self.max_tries):
            # 随机确定裁剪区域的宽度和高度
            crop_w_min = min(int(w * self.min_crop_side_ratio), int(self.size[0] * 0.5))
            crop_w_max = min(w, int(self.size[0] * 2))
            crop_w = w if crop_w_min >= crop_w_max else random.randint(crop_w_min, crop_w_max)

            crop_h_min = min(int(h * self.min_crop_side_ratio), int(self.size[1] * 0.5))
            crop_h_max = min(h, int(self.size[1] * 2))
            crop_h = h if crop_h_min >= crop_h_max else random.randint(crop_h_min, crop_h_max)
